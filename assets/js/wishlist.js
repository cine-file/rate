...
function renderWishlistItems(){
  ...
  list.innerHTML = wishlistItems.map(function(item, index){
    ...
    return '<div class="wishlist-item">' +
      ...
      (tv ? '' : '<div class="wishlist-item-score">' + (average === '' || average == null ? '—' : Number(average).toFixed(1)) + '<span>' + (count ? 'group avg (' + count + ')' : 'not rated yet') + '</span></div>') +
...
async function searchWishlist(query){
  ...
  try{
    ...
    const data = await apiCall(action, {token:getSessionToken(), query:query});
...
function renderWishlistResults(){
  ...
  dropdown.innerHTML = wishlistResults.map(function(item, index){
    ...
    return '<button class="wishlist-result" onclick="addWishlistResult(' + index + ')"><span class="wishlist-result-title">' + escHtml(String(title || '')) + '<br><small>' + escHtml(String(meta || '')) + '</small></span><span class="wishlist-add">Save</span></button>';
  }).join('');
  dropdown.classList.add('open');
}

async function addWishlistResult(index){
